/*
	Afin de faire fonctionner correctement les fonctions `Attack()` et `Defend()`,
	il faut que les variables passees en parametres soient des references.
	Il faut donc leur ajouter un `&`, afin de conserver les modifications faites a ces variables
	a l'interieur de la fonction.

	Il faut modifier les prototypes en consequence dans `GameManager.hpp`.

	Sans le `&`, les variables sont simplement copiees et les modifications
	dans la fonction ne sont faites que sur cette copie.
*/
